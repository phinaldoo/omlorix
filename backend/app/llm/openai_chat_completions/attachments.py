"""Chat Completions attachment conversion and upload handling.
...
def _impl_upload_files(
    ...
    """
    Uploads local user files to OpenAI for use with the Chat Completions API.
    Returns structured file parts ready for inclusion in `messages`.
    ...
    for file_id in file_ids:
        try:
            ...
            file_path_value = str(file_info.get("path") or "").strip()
            if (
                not file_name
                or not file_path_value
                or not os.path.exists(file_path_value)
            ):
