...
`ContextBuilder` considers system instructions, native tool schemas, workspace context, attached notes, memories, history, the current turn, and the requested output reserve together. It uses the configured model input limit, with a conservative 8,192-token fallback when a limit is unknown. Configure the actual model limits for production and BYOK deployments. Ollama's smaller `num_ctx` is respected and the chosen window is sent to Ollama. Context accounting does not inject output-token limits. OpenAI Responses, Chat Completions, Google AI Studio, OpenRouter, and Ollama requests omit optional output-limit parameters, including for nested tools, titles, and memory consolidation. Anthropic requests use the saved model’s admin-configured `max_tokens` unchanged; that required value is reserved during context accounting. Model `output_token_limit` metadata never clamps a request. Provider context and generation limits still apply.
