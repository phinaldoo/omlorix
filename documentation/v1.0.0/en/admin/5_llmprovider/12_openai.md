...
- **Service tier** offers **Flex**, **Standard**, and **Fast mode** where the model supports them. OpenAI renamed Priority processing to [Fast mode](https://developers.openai.com/api/docs/guides/fast-mode). Omlorix retains the supported `priority` API value and saved settings for compatibility; existing configurations and exports need no migration. Fast mode carries a per-token premium.
