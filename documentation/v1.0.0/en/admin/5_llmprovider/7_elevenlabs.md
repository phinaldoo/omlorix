...
The September 22, 2026 review adds [`scribe_v2_medical`](https://elevenlabs.io/docs/overview/models#scribe-v2-medical) alongside `scribe_v1` and `scribe_v2` for file transcription. It uses the same transcription API as Scribe v2. Speech-generation models continue to come from ElevenLabs' live `/v1/models` endpoint, including newly available models that advertise text-to-speech support. Realtime-only Scribe models are not offered for file transcription.
