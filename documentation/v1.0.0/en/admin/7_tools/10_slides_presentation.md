...
Review screenshots remain in model history to preserve cache prefixes while they fit the configured input budget. If needed, older screenshots are pruned without dropping the brief or latest visual feedback. At budget exhaustion, providers supporting tool-choice controls keep their schemas and disable calls; native Ollama still removes schemas. Retention increases input volume and does not guarantee lower cost. The [architecture guide](../../architecture/2_interactive_presentations.md#cache-stability-and-benchmark) includes the offline retention/pruning benchmark and its limitations.

The user-facing sidebar runs interactive HTML, including draft updates during generation, rather than displaying rendered slide images. Providers with streamed tool arguments can show partial initial HTML; exact edits appear once the complete candidate has been validated, before export rendering. A new HTML revision restarts that preview's interactive state; changing slides does not. Saved decks and editor changes load from HTML without waiting for rendered images. Images remain available for downloads and internal specialist visual review only. Preview documents use the same sandbox, expiry, quotas and cleanup as presentation playback.
...
Active presentation working directories are protected from temporary-file cleanup by process-held filesystem locks. Cleanup resumes automatically after normal completion or a worker crash. Deploy the updated backend to both rendering and maintenance workers; mixed versions do not provide this protection. Unexpected backend errors stop the specialist instead of consuming its remaining calls on retries. When no rendered revision exists, that non-retryable failure also prevents the parent model from restarting the same failed operation. Tool statistics distinguish failed receipts from successful updates.
