...
The always-on `email_worker` uses its own encrypted PostgreSQL outbox and is not one of the ten generic durable-queue consumers. Multiple replicas can safely claim work from that outbox. Keep the defaults unless queue telemetry and SMTP capacity justify a measured change:
