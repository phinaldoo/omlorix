...
Memories are maintained automatically from your chats; you do not need to review or confirm them. The page shows search and scope controls followed by individual memory cards, where you can inspect saved facts and optionally edit or delete them. Select **New Memory** if you want to add a fact yourself.
...
Facts receive a stability class and expire automatically without new supporting evidence: stable facts after 1,095 days, slow-changing facts after 540, changing facts after 180, and short-lived facts after 30. Supporting information in a normal chat refreshes the lifecycle automatically; you do not have to repeat facts to maintain memory. Optional manual edits also restart the lifecycle. Reads alone do not keep a fact alive. Expired facts are excluded immediately and removed by background maintenance. An explicit retraction can remove a fact sooner, and a high-value new fact can replace the weakest fact only when the 100-fact collection is full.

A model can ignore, misapply, duplicate, or create an inaccurate Memory. You can correct information naturally in chat or optionally edit an entry yourself. Deleting a Memory does not remove the same information from an earlier chat, Note, export, or external service.
